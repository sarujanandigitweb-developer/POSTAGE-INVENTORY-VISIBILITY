...
import { held, load, warmOthers } from '@/lib/client-datasets';
...
export default function Shell() {
  ...
  // PAGE AND SIZE LIVE HERE, WITH THE FILTERS. They used to be InventoryTab's own state,
  // which was fine while the browser held every row and cut its own page. It no longer
  // does: the server cuts the page, so the page number has to reach the request, and the
  // request is made here.
  const [st, setSt] = useState({ cat: 'CR', fam: '', sub2: '', attr: '', q: '', wh: '', st: '',
                                 sort: '', dir: 'asc', page: 1, size: '25' });
  // ANY change of what is being LOOKED FOR returns to page 1. Staying on page 40 while
  // narrowing a filter is how a reader lands on an empty table under a pager insisting
  // there are three pages. Changing the page itself, or the page size, must not reset it.
  const KEEP_PAGE = new Set(['page', 'size']);
  const set = patch => setSt(s => ({
    ...s, ...patch,
    ...(Object.keys(patch).every(k => KEEP_PAGE.has(k)) ? {} : { page: 1 }),
  }));
  ...
  const pickCategory = (key, value) => {
    if (value === '') {
      setSt(s => (s.cat === key ? { ...s, fam: '', page: 1 } : s));   // "Select" is not a filter state
      return;
    }
    setSt(s => ({ ...s, cat: key, fam: value === '*' ? '' : value, page: 1,
                  ...(s.cat !== key ? { sub2: '', attr: '' } : {}) }));
  };
  ...
  // NO LOCAL CACHE MAP HERE ANY MORE. It was keyed by category (and later by search term)
  // because a response WAS a category. A response is now one page of one filter/sort
  // combination, so the thing that identifies it is the whole URL — which is exactly what
  // lib/client-datasets.js keys on, and it already coalesces identical in-flight requests.
  ...
  useEffect(() => {
    ...
    // once: inventory-CR, which nobody was going to see, and slow-moving, which the reader
    // was waiting for. `booted` is in the test because `view` is 'inv' until the saved tab
    // is restored; without it the first frame fires the fetch this guard exists to prevent.
    if (!booted || view !== 'inv') return;

    // EVERY NARROWING TRAVELS WITH THE REQUEST. The browser no longer holds the rows to
    // narrow, so search, filters, sort, page and page size are all part of what is being
    // asked for. A parameter the reader has not set is left off entirely rather than sent
    // empty, which keeps the URL — and therefore the cache key — the same for the same
    // view of the data.
    const q = st.q.trim();
    const p = new URLSearchParams({ cat: st.cat, page: String(st.page), size: String(st.size) });
    if (q) p.set('q', q);
    for (const k of ['fam', 'sub2', 'attr', 'wh', 'st']) if (st[k]) p.set(k, st[k]);
    if (st.sort) { p.set('sort', st.sort); p.set('dir', st.dir || 'asc'); }
    const url = '/api/inventory?' + p;

    // A PAGE ALREADY FETCHED PAINTS AT ONCE, then is re-checked behind. Paging back and
    // forth is instant, and a tab left open all morning still moves on.
    const cached = held(url);
    ...
    // TYPING IS NOT A REQUEST PER KEYSTROKE — a search reads every section. Turning a page
    // or changing a filter is a deliberate act and goes immediately.
    if (!q) { go(); return () => { live = false; }; }
    const t = setTimeout(go, 300);
    return () => { live = false; clearTimeout(t); };
  }, [st.cat, st.q, st.fam, st.sub2, st.attr, st.wh, st.st, st.sort, st.dir, st.page, st.size,
      view, booted]);
  ...
  // COUNTED BY THE ROUTE, over the whole set. `data.rows` is one page now, so counting it
  // here would report the page's three out-of-stock SKUs as the category's total.
  const alerts = useMemo(
    () => (data && data.cat === st.cat && data.alerts) || { out: 0, low: 0 },
    [data, st.cat]);

  // EXPORT IS A DELIBERATE FULL READ, and the only one. The table is paginated, so the
  // rows on screen are 25 of however many match; exporting those would quietly hand the
  // reader a 25-row file and call it the filtered set. This re-asks for the same view with
  // `all=1`, which the route honours only for this.
  const exportCSV = async () => {
    if (!data) return;
    const p = new URLSearchParams({ cat: st.cat, all: '1' });
    if (st.q.trim()) p.set('q', st.q.trim());
    for (const k of ['fam', 'sub2', 'attr', 'wh', 'st']) if (st[k]) p.set(k, st[k]);
    if (st.sort) { p.set('sort', st.sort); p.set('dir', st.dir || 'asc'); }
    let full;
    try {
      full = await fetch('/api/inventory?' + p).then(r => r.json());
      if (!full.ok) throw new Error(full.error);
    } catch (e) { setErr('Export failed: ' + (e.message || e)); return; }
    ...
    // Already filtered and sorted by the route — filtering again here would be a second
    ...
    <div className="app">
      ...
      <div className="main">
        <Header view={booted ? view : null} asOf={booted ? data?.asOf : null} out={alerts.out} low={alerts.low}
                ...
                cat={st.cat} onCat={k => setSt(s => ({ ...s, cat: k, fam: '', sub2: '', attr: '', page: 1 }))}
